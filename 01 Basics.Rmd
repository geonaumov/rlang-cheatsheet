...
DUE TO THIS, A VARIABLE CCANNOT BE CALLED c BECAUSE THAT WILL OVERWRITE THE 
COLLECTION FUNCTION!!!
...
In this case, it will be appended n-times by the same value in order to become 
as long as the vector we are operating on. Then all values are calculated as in 
the previous example.

The example below multiplies everything in vec1 by 2, result is vector of same
size as vec1
```{r}
vec5 <- vec1 * 2 
```
...
length(vec6)
```
The vector is of type character because vectors can have only one data type,
text is the most complex one that can incorporate the rest. 
...
print(employee_list)
```
Here we have a list of vectors with some named values of different types,
a vector, and also 2 unnamed items: 12345 and "bla bla".
